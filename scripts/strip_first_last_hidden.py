...
This script reads one or more cached latents npz files and writes copies with that
block either:
...
Follows plot_umap.sh's two-file convention: pass both latents_pretrain.npz and
latents_hosts.npz, and each input basename is preserved in --out_dir.

Usage:
    python scripts/strip_first_last_hidden.py \\
        --in_npz   final_model/parallel_fixed/e110/latents_pretrain.npz \\
                   final_model/parallel_fixed/e110/latents_hosts.npz \\
        --out_dir  final_model/parallel_fixed/e110_no_first_last/ \\
        [--mode drop|zero]
"""
import argparse
