...
Prot predictions for the held-out fold. The result is an unbiased Prot prediction
for every star, made by a model that never saw that star during training.

Usage:
    python scripts/kfold_prot_inference.py \
        --model_path output/simple_rnn/models/baseline_v15_masking_real_60e.pt \
        --output_dir output/prot_predictor/kfold \
        --n_folds 10
"""
import argparse
import json
...
class ProtPredictor(nn.Module):
    """Simple MLP to predict log(Prot) from latent vectors + BPRP0."""
...
def load_data_fresh(model_path: str, h5_path: str, pickle_path: str, csv_path: str,
                    ...
                    max_samples: int = None, stratify_by_prot: bool = False,
                    n_prot_bins: int = 20, bprp0_min: float = None, bprp0_max: float = None,
                    use_metadata: bool = False, use_conv_channels: bool = False,
                    conv_config: dict = None,
                    device: torch.device = None):
    """Extract latent vectors from the model for Prot prediction."""
    ...
    # Apply BPRP0 cuts and require valid Prot
    valid_mask = ~np.isnan(all_prot) & (all_prot > 0) & ~np.isnan(all_bprp0)
    if bprp0_min is not None:
        valid_mask &= (all_bprp0 >= bprp0_min)
    if bprp0_max is not None:
        valid_mask &= (all_bprp0 <= bprp0_max)

    n_with_prot = np.sum(valid_mask)
    print(f'Stars with valid Prot and BPRP0: {n_with_prot}')

    if max_samples is not None:
        if stratify_by_prot:
            # Stratify by Prot instead of age
            prot_for_stratify = np.where(valid_mask, all_prot, np.nan)
            # Use log10(Prot) for stratification since Prot spans orders of magnitude
            log_prot = np.log10(np.where(prot_for_stratify > 0, prot_for_stratify, np.nan))
            sample_indices = get_stratified_indices(log_prot, max_samples, n_prot_bins)
        else:
    ...
    # Filter to valid Prot and BPRP0
    valid_mask = ~np.isnan(prot) & (prot > 0) & ~np.isnan(bprp0)
    ...
    print(f"Extracted {len(latent_vectors)} samples with valid Prot and BPRP0")
    ...
    return latent_vectors, prot, ages, bprp0, tic_ids
...
def train_single_fold(X_train, y_train, X_val, y_val,
    ...
    # Create model
    model = ProtPredictor(X_train.shape[1], hidden_dims, dropout).to(device)
...
def run_kfold_cv(latent_vectors, prot, bprp0, tic_ids,
    ...
    """Run k-fold cross-validation for Prot prediction.

    Args:
        save_models_dir: if provided, save fold models to this directory

    Returns:
        predictions: array of predicted Prot for all samples
        true_prot: array of true Prot
    ...
    y = np.log10(prot)  # Predict in log space
    ...
    for fold in range(n_folds):
        ...
        val_mae = np.mean(np.abs(10**val_pred_log - prot[val_idx]))
        val_corr = np.corrcoef(10**val_pred_log, prot[val_idx])[0, 1]

        print(f"  Fold {fold+1}/{n_folds}: train_loss={train_loss:.4f}, "
              f"val_MAE={val_mae:.2f} days, val_corr={val_corr:.3f}")
    ...
    return all_predictions, prot, tic_ids, fold_assignments, fold_losses, normalization_params


def plot_kfold_results(predictions, true_prot, bprp0, ages, tic_ids, fold_assignments, output_dir):
    """Create comprehensive plots for k-fold Prot prediction results."""

    # Compute metrics
    mae = np.mean(np.abs(predictions - true_prot))
    rmse = np.sqrt(np.mean((predictions - true_prot) ** 2))
    log_mae = np.mean(np.abs(np.log10(predictions) - np.log10(true_prot)))
    corr = np.corrcoef(predictions, true_prot)[0, 1]

    # Median absolute error (more robust)
    med_ae = np.median(np.abs(predictions - true_prot))

    # Log-space metrics
    log_residuals = np.log10(predictions) - np.log10(true_prot)
    log_bias = np.mean(log_residuals)
    log_scatter = np.std(log_residuals)

    print(f"\n=== K-Fold Cross-Validation Results ===")
    print(f"MAE: {mae:.2f} days")
    print(f"Median AE: {med_ae:.2f} days")
    ...
    # 1. Main scatter plot: predicted vs true Prot (log-log)
    ax1 = fig.add_subplot(2, 2, 1)
    scatter = ax1.scatter(true_prot, predictions, c=bprp0, cmap='coolwarm',
                          alpha=0.6, s=15, vmin=0.5, vmax=2.0)

    # Perfect prediction line
    min_prot, max_prot = true_prot.min(), true_prot.max()
    ax1.plot([min_prot, max_prot], [min_prot, max_prot], 'k--', lw=2, label='Perfect', zorder=10)

    # Factor of 2 lines
    ax1.plot([min_prot, max_prot], [min_prot*2, max_prot*2], 'k:', alpha=0.5, label='±2x')
    ax1.plot([min_prot, max_prot], [min_prot/2, max_prot/2], 'k:', alpha=0.5)

    ax1.set_xlabel('True Prot (days)', fontsize=12)
    ax1.set_ylabel('Predicted Prot (days)', fontsize=12)
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ax1.set_title(f'K-Fold Prot Predictions (r={corr:.3f})', fontsize=14)
    ...
    textstr = f'N = {len(true_prot)}\nMAE = {mae:.1f} days\nLog scatter = {log_scatter:.2f} dex\nr = {corr:.3f}'
    props = dict(boxstyle='round', facecolor='wheat', alpha=0.8)
    ax1.text(0.02, 0.98, textstr, transform=ax1.transAxes, fontsize=10,
             verticalalignment='top', bbox=props)

    # 2. Residuals vs true Prot
    ax2 = fig.add_subplot(2, 2, 2)
    residuals = predictions - true_prot
    ax2.scatter(true_prot, residuals, c=bprp0, cmap='coolwarm', alpha=0.6, s=15, vmin=0.5, vmax=2.0)
    ax2.axhline(0, color='k', linestyle='--', lw=2)
    ax2.set_xlabel('True Prot (days)', fontsize=12)
    ax2.set_ylabel('Residual (Pred - True) [days]', fontsize=12)
    ax2.set_xscale('log')
    ax2.set_title('Residuals vs True Prot', fontsize=14)
    ...
    if np.any(valid_ages):
        c = np.log10(np.where(valid_ages, ages, 1))
        scatter4 = ax4.scatter(bprp0, log_residuals, c=c, cmap='viridis', alpha=0.6, s=15)
        cbar4 = plt.colorbar(scatter4, ax=ax4)
        cbar4.set_label('log(Age/Myr)', fontsize=10)
    else:
        ax4.scatter(bprp0, log_residuals, alpha=0.6, s=15)
    ...
    results_df = pd.DataFrame({
        'TIC_ID': tic_ids,
        'true_prot_days': true_prot,
        'predicted_prot_days': predictions,
        'bprp0': bprp0,
        'age_myr': ages,
    ...
    # Save metrics
    metrics = {
        'n_samples': len(true_prot),
        'mae_days': float(mae),
        'median_ae_days': float(med_ae),
...
def main():
    parser = argparse.ArgumentParser(description='K-fold cross-validation for Prot prediction')
    ...
    parser.add_argument('--output_dir', type=str, default='output/prot_predictor/kfold', help='Output directory')
    ...
    parser.add_argument('--stratify_by_prot', action='store_true', help='Stratified sampling by Prot')
    parser.add_argument('--n_prot_bins', type=int, default=20, help='Number of Prot bins for stratification')
    ...
    latent_vectors, prot, ages, bprp0, tic_ids = load_data_fresh(
        ...
        stratify_by_prot=args.stratify_by_prot,
        n_prot_bins=args.n_prot_bins,
    ...
    predictions, true_prot, tic_ids, fold_assignments, fold_losses, norm_params = run_kfold_cv(
    ...
    metrics = plot_kfold_results(predictions, true_prot, bprp0, ages, tic_ids, fold_assignments, output_dir)

    # Save config
    config = vars(args)
    config['latent_dim'] = latent_vectors.shape[1]
    config['n_samples'] = len(prot)
