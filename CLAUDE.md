...
| `scripts/compute_saliency.py` | Per-timestep saliency for a single star. Integrated Gradients of `‖z‖²` and `z·PC1` w.r.t. (flux, flux_err) through the multiscale-pooled latent, plus the analytic minGRU write-gate magnitude. Writes `attribution.npz` + `sanity.json` per star into `output/saliency/`. |
| `scripts/plot_saliency.py` | 3-panel saliency figure from the npz: flux trace coloured by IG, flux_err trace coloured by IG, write-gate magnitude underneath. `--target {norm,pc1}`. |
| `saliency.sh` | Shell wrapper for saliency (model, star ID, IG steps, baseline mode all hardcoded). See `docs/plans/2026-06-27_saliency-mapping.md`. |
...
- **Per-timestep saliency (`docs/plans/2026-06-27_saliency-mapping.md`).** Integrated Gradients of `‖z‖²` and `z·PC1` w.r.t. (flux, flux_err) through the full multiscale-pooled latent. Default 64 IG steps, zero baseline; trapezoidal-midpoint completeness is ~5 % per star (intrinsic — the pool's argmax/argmin/first/last blocks make f piecewise-smooth along the α path, so midpoint Riemann converges as O(1/N) instead of O(1/N²); the per-timestep ranking is unaffected). Third panel shows the minGRU's write-gate magnitude `mean_d sigmoid(W_z x_t)_d` — non-decaying so structurally visible across long sequences. v1 confirms the user's prior that flux_err contributes much less mass than flux (Σ|s_err|/Σ|s_flux| ≈ 0.19 on the s86 smoke star).
