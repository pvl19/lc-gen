...
- **`--loso`** — leave-one-sector-out, **per-star** (`--star_aggregation latent_max`). `loso_sector_folds()` randomly partitions unique sectors into `--n_folds` disjoint groups; fold f holds out group f. VALIDATION = stars assigned to f (each validated once, in a group its sectors touch); TRAINING = only stars whose sectors are **all** outside group f — every star touching a held-out sector is removed from training entirely (whole star, not just its held-out-sector rows). Each star's sector-set is threaded through `latent_max` aggregation (rebuilt in `main` from `gaia_ids`→`sectors`, aligned to the aggregated rows). PCA is fit per-fold on training stars only (no projection leakage). Wrapper: `kfold_loso.sh` (picks PCA dim from the dim-sweep's best LOCO r, fallback 8; all labeled pretrain stars, cache ages). Verified by `tests/test_sector_mitigations.py`.
