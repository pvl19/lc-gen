...
def make_h5(path, n=32, L=512):
    ...
    with h5py.File(path, 'w') as f:
        ...
        grp.create_dataset('median_flux',   data=rng.uniform(1e4, 1e5, n).astype(np.float32))
        grp.create_dataset('iqr_half_flux', data=rng.uniform(10, 1000, n).astype(np.float32))
